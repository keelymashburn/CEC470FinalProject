...
{
	// this code initializes the values used below 
    bool hit = false; 
	// uses formulas to caluclate tag, cache line, and cache word
    int tag = ((address & tag_mask) >> (int) (log2(number_of_words_in_block)+log2(number_of_lines_in_cache)));
	int cache_line = ((address & line_mask) >> (int) log2(number_of_words_in_block));
	int cache_word = address & word_mask; 

	// increments # of memory requests 
	number_of_memory_requests++;

	// if it finds it, there is a hit
	if (tags[cache_line] != -1)
	{
		// increment total number of hits
		hit = true;
		number_of_hits++;
	}
	// not found, there is a miss 
	else 
	{
		// set the tags 
		tags[cache_line] = tag;
...
unsigned int DirectCache::getPercentageOfHits(void)
{
	...
	// uses given formula to calculate percentage
